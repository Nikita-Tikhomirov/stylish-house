<section class="prodMain wrapper catCalculator" style="padding-top: 40px;">
    ...
    <div class="prodForm">
            ...
            <div class="prodForm__galleryWrap">
                <div class="prodForm__imgWrap">
                    ...
                    @if (!empty($product->fabric_photo))
                        <img src="{{ asset(rawurlencode($product->fabric_photo)) }}" alt="{{ $product->h1 }} материал" />
                    @endif
                </div>
                ...
                <div class="prodForm__bar">
                    @foreach ($sameModelProducts as $sameProduct)
                        ...
                        @elseif($sameProduct->fabric_photo)
                            <img src="{{ asset(rawurlencode($sameProduct->fabric_photo)) }}" alt="{{ $sameProduct->h1 }} материал" />
                        @endif
                    @endforeach
                </div>
            </div>
        ...
        <div class="prodForm__calcFormWrap">
            <div class="prodForm__formSubtitle">{{ $category->titleh1 ?? 'Рольставни' }}</div>
            <div class="prodForm__formTitle">{{ $product->h1 ?? 'Рольставни' }}</div>
            <x-front.read-more :text="$product->first_screenn_description ?? ''"
                :id="'rollets-calculator-summary-'.$product->id" />

            <input type="hidden" name="modelSelect" class="modelSelect" value="{{ $product->model_title ?? ($product->h1 ?? 'Рольставни') }}">
            <input type="hidden" name="cloth" class="cloth" value="{{ $product->cloth ?? '' }}">
            <input type="hidden" name="model" class="model" value="{{ $product->model_title ?? ($product->h1 ?? 'Рольставни') }}">
                    ...
                    <div class="accordion-item">
                        ...
                        <div class="accordion-content">
                            ...
                            <label class="option-label">
                                ...
                                <span class="option-name">Воротковый (кардан)</span>
                            ...
                            <label class="option-label">
                                ...
                                <span class="option-name">Пружинно-инерционный механизм</span>
                            ...
                            <label class="option-label">
                                ...
                                <span class="option-name">Электропривод</span>
                                <span class="option-price">+{{ $product->electric_price ?? 0 }}₽</span>
                            </label>
                        </div>
                    </div>
                    ...
                    <div class="accordion-item">
                        ...
                        <div class="accordion-content">
                            <label class="option-label">
                                ...
                                <span class="option-name">Ригельный замок</span>
                                <span class="option-price">+{{ $product->rigel_price ?? 0 }}₽</span>
                            </label>
                            <label class="option-label">
                                ...
                                <span class="option-name">Щеколда</span>
                                <span class="option-price">+{{ $product->shchyolka_price ?? 0 }}₽</span>
                            </label>
                            <label class="option-label">
                                <input type="radio" name="lock-device" value="upper" data-price="{{ $product->upper_price ?? 0 }}" {{ $product->lock_device == 'upper' ? 'checked' : '' }}>
                                <span class="option-name">Верхний ригель</span>
                                <span class="option-price">+{{ $product->upper_price ?? 0 }}₽</span>
                            </label>
                            <label class="option-label">
                    ...
                    <div class="accordion-item">
                        ...
                        <div class="accordion-content">
                            <label class="option-label">
                                ...
                                <span class="option-price">+{{ $product->ral_price ?? 0 }}₽</span>
                            </label>
                            <label class="option-label">
                                <input type="checkbox" name="photo-print" value="photo-print" data-price="{{ $product->photo_price ?? 0 }}" {{ $product->photo_print ? 'checked' : '' }}>
                                <span class="option-name">Фотопечать</span>
                                <span class="option-price">+{{ $product->photo_price ?? 0 }}₽</span>
                            </label>
                        </div>
                    </div>
                </div>
            ...
            <div class="prodForm__priceAndAddToCart">
                <div class="prodForm__price">Цена: 0₽</div>
                <button class="prodForm__addToCart" data-id="{{ $product->id }}"> Добавить в корзину </button>
            </div>
        </div>
    </div>
</section>
