...
При нажатии `Продолжить` сохранённый каталог `browser-seed` автоматически проверяется и переносится в основной checkpoint до запуска Playwright. Повторный перенос идемпотентен: уже принятые товары, фотографии и связи с категориями не дублируются. Повреждённый JSON, отсутствующая фотография или неверный WebP останавливают перенос с диагностикой до обращения к донору.
