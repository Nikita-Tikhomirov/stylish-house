                    ...
                    @forelse ($installationTypes as $item)
                        <form class="installation-type-card mb-4 p-3 border rounded" data-id="{{ $item->id }}" enctype="multipart/form-data">
                            ...
                            <div class="form-group">
                                ...
                                @if ($item->image)
                                    <img src="{{ Storage::url($item->image) }}" style="max-width: 180px; margin-top: 10px;" alt="Монтаж">
                                @endif
                            </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function initInstallationEditor(card) {
            ...
            if (!textarea || !editorEl || editorEl.dataset.inited === '1') {
                return;
            }

            const quill = new Quill(editorEl, {
                modules: { toolbar: installationToolbar },
                theme: 'snow'
            });

            if (textarea.value && !editorEl.innerHTML.trim()) {
        ...
        container.querySelectorAll('.installation-type-card').forEach((card) => {
            initInstallationEditor(card);
        });
