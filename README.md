...
Runtime data must use an explicit absolute writable root outside Windows drive `C:`. On this workstation use `G:\stylish-house-data\rimskie-imports`. You can pass it on every command or set `RIMSKIE_IMPORT_DATA_ROOT`. The CLI rejects relative paths, unwritable directories, Windows device namespaces, every spelling of drive `C:`, and paths whose existing junction or reparse target escapes the selected root. The run directory and its state, source, product, image, profile, control, event, membership, and export children are checked before runtime files are opened or created.
