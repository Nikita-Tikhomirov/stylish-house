...
Если карточки сохраняются через рабочую браузерную сессию, панель автоматически показывает их фактическое количество, фотографии, текущий товар и время следующего запроса. После перезапуска панели уже сохранённые карточки остаются видны и доступны в списке товаров.

При нажатии `Продолжить` сохранённый каталог `browser-seed` автоматически проверяется и переносится в основной checkpoint до запуска Playwright. Вместе с товарами переносится часовой журнал обращений, поэтому смена браузерной сессии не сбрасывает лимит. Повторный перенос идемпотентен: уже принятые товары, фотографии и связи с категориями не дублируются. Повреждённый JSON, отсутствующая фотография или неверный WebP останавливают перенос с диагностикой до обращения к донору.
...
Короткая пользовательская инструкция и карта файлов находятся в [`docs/rimskie-import-operator-guide.md`](docs/rimskie-import-operator-guide.md).
